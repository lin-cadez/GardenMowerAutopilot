...
- ALWAYS mirrors CH1 and CH2 to outputs (CH1_OUT, CH2_OUT).
- If AUTOPILOT mode is ON (CH5 > 1900us) AND camera detects a target,
  outputs are forced to NEUTRAL (1500us) = STOP.
- If AUTOPILOT mode is OFF (CH5 < 1100us), user has full control.

Notes:
- Fixed variable typos: target_detected / tagret_detected.
- Control loop now mirrors RC continuously (not only on mode transitions).
"""
...
def control_loop():
    ...
    while True:
        ...
        # Always mirror RC EXCEPT when autopilot ON and target detected => STOP
        if autopilot_mode and target_detected:
            pi.set_servo_pulsewidth(CH1_OUT, NEUTRAL_PULSE)
            pi.set_servo_pulsewidth(CH2_OUT, NEUTRAL_PULSE)
        else:
